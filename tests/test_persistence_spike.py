...
class PersistenceSpike(unittest.TestCase):
    ...
    def test_reservation_survives_round_trip(self):
        original = create_reservation(PLACE, ALICE, T0, T0 + timedelta(hours=2))
        confirm(original, [])
    ...
    def test_loaded_reservations_feed_the_overlap_rule(self):
        repo = ReservationRepository(self.path)
        a = create_reservation(PLACE, ALICE, T0, T0 + timedelta(hours=2))
        confirm(a, [])
        repo.save(a)

        b = create_reservation(PLACE, ALICE, T0 + timedelta(hours=1), T0 + timedelta(hours=3))
        with self.assertRaises(RuleViolation):
            confirm(b, repo.for_place(PLACE.id))
        repo.close()
